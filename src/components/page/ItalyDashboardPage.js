...
class ItalyDashboardPage extends React.Component {
    ...
    generateBlock = (block, data) => {
        ...
        selectedCountry.forEach((c,id)=>{
          const date = new Date(c.data.split('T')[0]);
          c.date = date.getTime();
        ...
        const cur_nazione = nazione.filter(p=>(new Date(p.data.split('T')[0]).getTime() == latest_date))[0];
        const yesterday_nazione = nazione.filter(p=>(new Date(p.data.split('T')[0]).getTime() == yesterday_date))[0];

        const cur_regioni = regioni.filter(p=>(new Date(p.data.split('T')[0]).getTime() == latest_date));
        const selectedCountryLatest = selectedCountry.filter(p=>(new Date(p.data.split('T')[0]).getTime() == latest_date))[0];
        const selectedCountryYesterday = selectedCountry.filter(p=>(new Date(p.data.split('T')[0]).getTime() == (latest_date - 24 * 3600 * 1000)))[0];

        const pointList = province.filter(p=>(new Date(p.data.split('T')[0]).getTime() == latest_date)).map(
          (prov) => {
