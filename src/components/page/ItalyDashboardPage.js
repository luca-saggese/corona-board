...
class ItalyDashboardPage extends React.Component {
    ...
    generateBlock = (block, data) => {
        ...
        province.forEach(p=>{
          latest_date=Math.max(latest_date, new Date(p.data).getTime())
          max_casi_provincia=Math.max(max_casi_provincia, p.totale_casi)
        });
        
        const yesterday_date = new Date(latest_date) - 24 * 3600 * 1000;
        const cur_nazione = nazione.filter(p=>(new Date(p.data).getTime() == latest_date))[0];
...
console.log('cur_regioni')
        ...
        const pointList = province.filter(p=>(new Date(p.data).getTime() == latest_date)).map(
          (prov) => {
